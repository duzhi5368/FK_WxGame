...
Page({
  ...
  onLoad: function(options) {
    ...
    //接受参数
    that.setData({
        sumprice: (parseFloat(options.sumprice)).toFixed(2),
        amount:(parseFloat(options.sumprice)).toFixed(2)
    })
    //默认时间部分
    that.inittime();
    //配送时间部分
  ...
  postorder: function() {
    ...
    if (addressid == 0) {
      ...
      //购买的商品参数
      var productList = [], cartIds="";
      var goodslist = that.data.cakelist;
      for (var i = 0; i < goodslist.length; i++) {
        productList[i] = {
          id: goodslist[i].product_spec_id, //商品id
          price: parseFloat(goodslist[i].price), //商品单价
          fAmount: parseInt(goodslist[i].famount), //商品数量
          remark: goodslist[i].remark, //备注
          name: goodslist[i].name, //商品名称
          spec: goodslist[i].spec,//购买规格
          fsum: parseFloat(goodslist[i].price) * parseInt(goodslist[i].famount),//小计
        }
        cartIds += goodslist[i].id+",";
      }
      ...
      var params = {
        ...
        stroeId: that.data.sendtype == 1?"":that.data.shopid, //门店id
        productList: productList, //购买的商品
      };
